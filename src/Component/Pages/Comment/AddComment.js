...
const AddComment = ({ show, onClose, selectedResource }) => {
   ...
   const handleAddComment = async (e) => {
     ...
     try {
       const res = await axios.post(`http://5.180.148.40:9001/api/case-man/v1/comment/${selectedResource.row_id}`, {
   ...
   const handleDelete = async (id) => {
     const comment_id = id;
     try {
       const response = await axios.delete(`http://5.180.148.40:9001/api/case-man/v1/comment/${selectedResource.row_id}`, {
         data: { comment_id },
         headers: {
           "Content-Type": "application/json",
           Authorization: `Bearer ${token}`,
     ...
     <div className="popup-overlay" style={{padding:'150px 100px 0px 0px'}}>
       ...
       {showUpdatePopup && commentToEdit && (
         <UpdateComment 
           ...
           resourceId={selectedResource.row_id}
         />
       )}
     </div>
   );
