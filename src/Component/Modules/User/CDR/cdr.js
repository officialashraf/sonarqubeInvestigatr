...
//       {activePopup === 'filter' && <AddFilter2 togglePopup={closeAllPopups} />}
...
const Cdr = ({ togglePopup }) => {
  const [selectedOption, setSelectedOption] = useState('localStorage');
  const [selectDataType, setSelectDataType] = useState('CDR');
  const [activePopup, setActivePopup] = useState(null); // 🔹 Manage active popups
  ...
  const handleProceed = () => {
    if (selectedOption === 'osintData') {
      setActivePopup('filter'); // Open AddFilter2 and close others
    } else if (selectedOption === 'ftpServer') {
      setActivePopup('ftp'); // Open FTP popup without affecting others
    } else if (selectedOption === 'localStorage') {
      setActivePopup('upload'); // Open file upload popup without affecting others
    }
  };
  ...
  const closePopup = (popup) => {
    ...
    if (popup === 'filter') {
      setActivePopup(null); // Close AddFilter2
    }
  };
    ...
    <>
      <div className={styles.popupOverlay}>
        <div className={styles.popupContainerCdr}>
          ...
          <div className={styles.popupBodyCdr}>
            ...
            <div className={styles.optionsListCdr}>
              {options.map((option) => (
                <label
                  key={option.value}
                  className={`${styles.optionItemCdr} ${
            ...
            <div className={styles.targetDataTypeCdr}>
              <div className={styles.selectDataTypeCdr}>
                <div className={styles.customOutlinedWrapper}>
                  ...
                  <small>
                    Selected Data Type would be applied to all resources in this batch
                  </small>
                </div>
              </div>
            </div>
          ...
          <div className={styles.popupFooterCdr}>
            <button className={styles.cancelBtnCdr} onClick={togglePopup}>
              CANCEL
            </button>
            <button className={styles.proceedBtnCdr} onClick={handleProceed}>
              PROCEED
            </button>
          </div>
        </div>
      ...
      {activePopup === 'filter' && (
        <AddFilter2 togglePopup={() => closePopup('filter')} />
      )}
      {activePopup === 'upload' && <FileUpload togglePopup={() => setActivePopup(null)} />}
      {activePopup === 'ftp' && <FtpPopup togglePopup={() => setActivePopup(null)} />}
    </>
  );
};
