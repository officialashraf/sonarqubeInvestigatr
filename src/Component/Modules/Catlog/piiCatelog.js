...
const PIICatelog = () => {
    ...
    const fetchCatalog = async () => {
        try {
            ...
            const response = await axios.get(`${window.runtimeConfig.REACT_APP_API_OSINT_MAN}/api/osint-man/v1/pii-catalogues`, {
                headers: {
    ...
    const handleSaveChanges = async (editedRows) => {
        ...
        try {
            await axios.put(
                `${window.runtimeConfig.REACT_APP_API_OSINT_MAN}/api/osint-man/v1/pii-catalogues/batch-update`,
                { rows: rowsToUpdate },
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
