...
export const taskFilterReducer = (state = initialState, action) => {
  ...
  const initialState1 = { data: [], headers: [], page: 1, totalPages: 0, totalResults: 0, loading: false, error: null };
  ...
  export const summaryDataReducer = (state = initialState1, action) => {
    switch (action.type) {
      ...
      case SET_PAGINATION:
        return { ...state, page: action.payload.page, totalPages: action.payload.totalPages, totalResults: action.payload.totalResults };
      case SET_LOADING:
